...
export function usePDFGenerator() {
  ...
  const generateTestResultPDF = async (testResult: TestResultWithDetails, questions: Question[], userAnswers: number[]) => {
    ...
    try {
      ...
      // Функция для генерации HTML с ограничением блоков на странице (максимум 2)
      const generateHTMLWithPageBreaks = () => {
        const maxBlocksPerPage = 2
        let html = `
        ...
        if (questions.length > 0) {
          ...
          questions.forEach((question, index) => {
            // Добавляем разрыв страницы каждые 2 блока
            if (index > 0 && index % maxBlocksPerPage === 0) {
              html += `<div style="page-break-before: always;"></div>`
            }
            
            html += `
              <div style="margin-bottom: 40px; page-break-inside: avoid; padding-bottom: 20px; border-bottom: 1px solid #f0f0f0;">
          ...
          // Для БПД теста без детальных вопросов
          html += `
            <div>
              <h2 style="font-size: 16px; margin: 0 0 20px 0; color: #374151; font-weight: 600; page-break-inside: avoid;">Детальные результаты</h2>
                  ...
                  ${userAnswers.map((answer, index) => {
                    ...
                    if (index > 0 && index % maxBlocksPerPage === 0) {
                      return `<div style="page-break-before: always;"></div>
                        <p style="margin: 8px 0; padding: 10px; background-color: #f8fafc; border-radius: 6px; border: 1px solid #e5e7eb; font-size: 13px; line-height: 1.4;">
                          <strong>${questions.length > index ? questions[index].text : `Вопрос ${index + 1}`}:</strong><br/>
                          <span style="color: #4b5563; margin-left: 10px;">${answer >= 0 ? ['Никогда', 'Редко', 'Иногда', 'Часто', 'Всегда'][answer] : 'Не отвечено'}</span>
                        </p>`
                    }
      ...
      // Генерируем HTML контент с ограничением блоков на странице
      tempDiv.innerHTML = generateHTMLWithPageBreaks()
      ...
      // Улучшенная логика разбивки страниц с правильными отступами
