...
export function useUserHasPaid() {
  ...
  const getUserHasPaid = async (userId: string): Promise<boolean> => {
    ...
    try {
      ...
      // ВРЕМЕННО: всегда возвращаем false для новых авторизаций
      // Статус оплаты должен устанавливаться только после реальной оплаты
      console.log('✅ Статус оплаты из БД:', data?.haspaid)
      console.log('🔄 ВРЕМЕННО: Возвращаем false для новых авторизаций')
      return false
    } catch (err) {
